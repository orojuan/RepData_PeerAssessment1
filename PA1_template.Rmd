...
According to the data, the mean total number of steps taken per day is **`r mean(daily)`**, while the median is **`r median(daily)`**. This shows a negative skew in the distribution.
...
xyplot(avginter~as.numeric(names(avginter)),type="l",main="Daily activity pattern",xlab="Interval",ylab="Average steps")
```

From the previous plot, we can see that the interval with the maximum average steps is **`r names(which.max(avginter))`**.


## Imputing missing values

There are a total of **`r sum(is.na(mydf$steps))`** missing values (coded as NA). The presence of missing days may introduce bias into some calculations or summaries of the data. Therefore, we have filled in all of those by using the rounded mean of the  corresponding 5-minute intervals:
...
Please notice that this time, both the mean **`r mean(daily2)`** and the median **`r median(daily2)`** are almost the same, suggesting a symmetric distribution (Skewness has disappeared).
...
xyplot(jn~as.numeric(sn[seq(length(jn))%%2==1])|sn[seq(length(jn))%%2==0],type="l",main="Daily activity pattern by weekday/weekend",xlab="Interval",ylab="Average steps")
```
